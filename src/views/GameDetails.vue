                ...
                <div v-if="platformList.length > 0" class="flex items-center justify-between gap-2">
                  ...
                  <div class="flex flex-wrap items-center justify-end gap-2">
                    <span v-for="p in platformList" :key="p" class="tooltip tooltip-top flex items-center justify-center" :data-tip="p" :title="p">
                        <PlatformIcon :name="p" class="w-5 h-5 sm:w-6 sm:h-6" />
                      </span>
                  </div>
                </div>
...
export default {
  ...
  setup(props) {
    ...
    const PLATFORM_ORDER = [
      'PC', 'PlayStation 5', 'PlayStation 4', 'Xbox Series X/S', 'Xbox One',
      'Nintendo Switch', 'Nintendo Switch 2', 'Steam', 'iOS', 'Android',
      'macOS', 'Linux', 'Other'
    ];
    ...
    const platformList = computed(() => {
      ...
      if (game.value?.platforms_list) {
        try {
          const parsed = JSON.parse(game.value.platforms_list);
          if (Array.isArray(parsed) && parsed.length > 0) list = parsed;
        } catch {
          // fallthrough
        }
      }
      if (list.length === 0 && game.value?.platform) {
        list = [game.value.platform];
      }
      return list
        .map(p => p.trim())
        .filter(Boolean)
        .slice()
        .sort((a, b) => {
          const ia = PLATFORM_ORDER.indexOf(a) === -1 ? PLATFORM_ORDER.length : PLATFORM_ORDER.indexOf(a);
          const ib = PLATFORM_ORDER.indexOf(b) === -1 ? PLATFORM_ORDER.length : PLATFORM_ORDER.indexOf(b);
          return ia - ib;
        });
    });
