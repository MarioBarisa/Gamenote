<template>
  <div class="news-carousel bg-base-200 rounded-lg p-6">
    <h2 class="text-2xl font-bold mb-4">Gaming Vijesti</h2>
    ...
    <div v-else-if="currentNews" class="relative">
      ...
      >
        <div class="flex gap-4">
          <div v-if="currentNews.enclosure?.link || currentNews.thumbnail" class="flex-shrink-0">
            <img 
      ...
      <!-- inidkacijski krugovi za shuffler -->
  ...
  setup() {
    ...
    const startCarousel = () => {
      ...
      if (news.value.length > 1) {
        ...
        }, 5000); // 5 sekundi za shuffle
      }
    };

    const openNewsDetail = (newsItem) => {
      // Provjeri da li je vanjski link
      if (newsItem.link && newsItem.link !== '#') {
        // Otvori vanjski link u novom tabu
        window.open(newsItem.link, '_blank', 'noopener,noreferrer');
      } else {
        // locall storage za prikaz kada user click-a
